...
import { Target, User, LogOut, Star, CalendarCheck, ClipboardCheck, MessageSquareWarning, Users, X, CalendarDays, BriefcaseBusiness } from "lucide-react";
...
export default function SidebarXodim() {
  ...
  const navItems = [
    ...
    ...(role === "kadr"
      ? [
          ...
          { href: "/xodim/kalendar", icon: CalendarDays,         label: "Kalendar" },
          { href: "/xodim/ish-staji", icon: BriefcaseBusiness,   label: "Ish staji" },
        ]
      : []
    ),
  ...
  return (
    <>
      <MobileTopBar accent={accent} letter="T" onOpen={() => setMobileOpen(true)} />
      ...
      <aside className={`fixed left-0 top-0 bottom-0 flex flex-col z-50 transition-transform duration-300 lg:translate-x-0 ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}
