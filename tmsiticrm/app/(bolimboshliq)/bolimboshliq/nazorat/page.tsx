...
function DetailPanel({
  ...
  row: DocBolimRow;
  onQaror: (holati: BolimHolati, izoh: string) => Promise<void>;
}) {
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5 flex flex-col gap-5">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="p-3" style={{ background: "#F4F9FD", borderRadius: 12 }}>
            <p className="text-xs font-bold mb-1" style={{ color: "#91929E" }}>Yuborilgan sana</p>
            <p className="text-sm font-bold" style={{ color: "#0A1629" }}>{fmt(row.assigned_at)}</p>
          </div>
        </div>
        ...
        {isPending && (
          <div className="p-4" style={{ background: "#F4F9FD", borderRadius: 14 }}>
            <p className="text-xs font-bold mb-2 uppercase tracking-wide" style={{ color: "#91929E" }}>Qaror va izoh</p>
            <textarea
              value={izoh} onChange={e => setIzoh(e.target.value)}
              placeholder="Ixtiyoriy izoh (qabul qilsangiz ham, rad etsangiz ham yozishingiz mumkin)..."
              rows={3} className="w-full px-3 py-2.5 text-sm outline-none resize-none"
              style={{ background: "#FFFFFF", borderRadius: 10, border: "1.5px solid #EEF2FF", color: "#0A1629" }}
            />
            <div className="flex gap-3 mt-3">
              <button onClick={() => handleAction("qabul_qilindi")} disabled={saving}
                className="flex-1 flex items-center justify-center gap-2 py-3 text-sm font-bold text-white disabled:opacity-50"
                style={{ background: "#3F8CFF", borderRadius: 12, boxShadow: "0 4px 12px rgba(63,140,255,0.3)" }}>
                {saving ? <Loader2 size={14} className="animate-spin" /> : <CheckCircle2 size={14} />}
                Qabul qilish
              </button>
              <button onClick={() => handleAction("rad_etildi")} disabled={saving}
                className="flex-1 flex items-center justify-center gap-2 py-3 text-sm font-bold disabled:opacity-50"
                style={{ background: "#FFFFFF", border: "2px solid #FF5C5C", color: "#FF5C5C", borderRadius: 12 }}>
                <XCircle size={14} />
                Menga taalluqli emas
              </button>
            </div>
          </div>
        )}
...
export default function BolimNazoratPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const data = await apiFetch<DocBolimRow[]>("/ijro-docs/bolim-inbox");
      setRows(data);
  ...
  async function handleQaror(rowId: number, holati: BolimHolati, izoh: string) {
    await apiFetch(`/ijro-docs/bolim-inbox/${rowId}/qaror`, {
      method: "POST",
      body: JSON.stringify({ holati, izoh }),
    });
    await load();
  }
  ...
  const filtered = rows.filter(r => {
    ...
    if (search) {
      ...
      return (r.bolim_nomi || "").toLowerCase().includes(s);
    }
    return true;
  });
  ...
  const counts = {
    all:               rows.length,
    yuborildi:         rows.filter(r => r.holati === "yuborildi").length,
    qabul_qilindi:     rows.filter(r => r.holati === "qabul_qilindi").length,
          ...
          <div className="px-4 pt-4 pb-3 flex-shrink-0" style={{ borderBottom: "1px solid #F4F9FD" }}>
            ...
            <div className="flex flex-wrap gap-1.5">
              {[
                ...
                { key: "qabul_qilindi",     label: `Qabul (${counts.qabul_qilindi})` },
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 px-4 text-center">
                ...
                <p className="text-xs mt-1" style={{ color: "#91929E" }}>Bu bo'limga hozircha hujjat yuborilmagan</p>
              </div>
            ) : filtered.map(r => {
              const cfg = HOLATI_CFG[r.holati];
              const isNew = r.holati === "yuborildi";
              return (
                <div key={r.id}
                  ...
                  <div className="flex items-center gap-2 mb-1.5">
                    {isNew && (
                      <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: "#3F8CFF" }} />
                    )}
